...
class SlackImporter(ZulipTestCase):
    ...
    def test_end_to_end_slack_import(
        ...
        with (
            mock.patch(
                "zerver.worker.deferred_work.save_attachment_contents"
            ) as mocked_save_attachment,
            mock.patch("zerver.worker.deferred_work.do_convert_zipfile") as mocked_convert_zipfile,
            mock.patch(
                "zerver.worker.deferred_work.do_import_realm", return_value=realm
        ...
        # Confirmation key at this point is marked used but since we are mocking the
        # process, we need to do it manually here.
        ...
        UserProfile.objects.create(realm=realm, delivery_email=email)
        ...
        result = post_process_request()
        self.assertEqual(result.status_code, 302)
        self.assertEqual(result.url, get_safe_redirect_to(reverse("login"), realm.url))  # type: ignore[attr-defined]
