...
class UnreadDirectMessageCounter {
    ...
    get_counts(include_max_msg_id = false) {
...
class UnreadTopicCounter {
    ...
    get_counts(include_per_topic_count = false, include_per_topic_max_msg_id = false) {
        const res = {};
        ...
        for (const [stream_id, per_stream_bucketer] of this.bucketer) {
            ...
            if (include_per_topic_count) {
                ...
                for (const [topic, msgs] of per_stream_bucketer) {
                    ...
                    if (include_per_topic_max_msg_id) {
...
export function get_unread_pm(include_per_bucket_max_msg_id = false) {
    const pm_res = unread_direct_message_counter.get_counts(include_per_bucket_max_msg_id);
    return pm_res;
}

export function get_unread_topics(include_per_topic_max_msg_id = false) {
    const include_per_topic_count = true;
    const topics_res = unread_topic_counter.get_counts(include_per_topic_count, include_per_topic_max_msg_id);
    return topics_res;
}
