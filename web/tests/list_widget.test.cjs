    ...
    function row(item) {
        return {
            ...
            prev() {
                ...
                return index > 0 ? row(rows[index - 1]) : {length: 0, is: () => false};
            },
            is($other) {
...
run_test("render_item moves a row to its item's new position", () => {
    ...
    // Item 6 now sorts between items 50 and 51, and item 70 first; item 7
    // stays where it is.
    list[5].key = 50.5;
    list[69].key = 0.5;
    widget.filter_and_sort();
    widget.render_item(list[5]);
    widget.render_item(list[69]);
    widget.render_item(list[6]);
    assert.equal(rows[rows.indexOf(list[49]) + 1], list[5]);
    assert.equal(rows[0], list[69]);
    assert.deepEqual(widget.get_rendered_list(), rows);
    // Only item 7's row was replaced in place; the other two moved, and
