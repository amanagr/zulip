...
class PickBasePortTest(unittest.TestCase):
    ...
    def test_honors_base_port_env_without_probing(self) -> None:
        # When BASE_PORT is set (typically by a per-worktree
        # devenv.local.nix that shifted every service port), trust it
        # as-is and skip the fallback probe.  The fallback range
        # starts at the main checkout's 9991, so a silent fallback
        # would land in the wrong worktree's port space.
        with (
            patch.dict("os.environ", {"BASE_PORT": "10491"}, clear=False),
            patch("tools.lib.run_dev_helpers.is_port_range_available") as mocked,
        ):
            self.assertEqual(pick_base_port(None, None), 10491)
            mocked.assert_not_called()

    def test_invalid_base_port_env_falls_through(self) -> None:
        # A garbage BASE_PORT value shouldn't raise; fall back to the
        # normal probe so the user still gets a working dev server.
        with (
            patch.dict("os.environ", {"BASE_PORT": "garbage"}, clear=False),
            patch("tools.lib.run_dev_helpers.is_port_range_available", return_value=True),
        ):
            self.assertEqual(pick_base_port(None, None), DEFAULT_BASE_PORT)

    def test_explicit_request_beats_base_port_env(self) -> None:
        # --base-port is the highest-priority signal; it must win
        # over BASE_PORT, otherwise a user explicitly overriding from
        # the command line would be silently ignored.
        with patch.dict("os.environ", {"BASE_PORT": "10491"}, clear=False):
            self.assertEqual(pick_base_port(8080, None), 8080)


class TestBasePortShiftTest(unittest.TestCase):
    def test_test_base_port_shifts_with_runtime_base_port(self) -> None:
        # TEST_BASE_PORT is computed at module import time from the
        # ambient BASE_PORT env var; reload the module under a shifted
        # BASE_PORT and check the offset propagates.
        import importlib

        import tools.lib.run_dev_helpers

        with patch.dict("os.environ", {"BASE_PORT": "10491"}, clear=False):
            reloaded = importlib.reload(tools.lib.run_dev_helpers)
            self.assertEqual(reloaded.TEST_BASE_PORT, DEFAULT_TEST_BASE_PORT + 500)

        # Restore module state for any later tests.
        importlib.reload(tools.lib.run_dev_helpers)

    def test_malformed_base_port_env_does_not_crash_import(self) -> None:
        # _runtime_base_port is computed at module load time before
        # tools/run-dev gets a chance to reject the bad value, so the
        # import path must tolerate a garbage env var.  An in-process
        # patch.dict + importlib.reload would mask a regression here:
        # the module body runs at the very top of the reload, and a
        # ValueError there would propagate even after reload restored
        # the patched env.  Run a fresh interpreter to actually exercise
        # the load-time codepath.
        repo_root = os.path.dirname(
            os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        )
        env = {**os.environ, "BASE_PORT": "garbage"}
        result = subprocess.run(
            [
                sys.executable,
                "-c",
                "from tools.lib.run_dev_helpers import "
                "DEFAULT_BASE_PORT, TEST_BASE_PORT, _runtime_base_port; "
                "print(_runtime_base_port, TEST_BASE_PORT)",
            ],
            cwd=repo_root,
            env=env,
            capture_output=True,
        ...
        runtime_base, test_base = result.stdout.split()
        self.assertEqual(int(runtime_base), DEFAULT_BASE_PORT)
        self.assertEqual(int(test_base), DEFAULT_TEST_BASE_PORT)
