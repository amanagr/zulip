...
``--generate-stripe-fixtures`` is passed).  Fixtures live in
``corporate/tests/stripe_fixtures/``, one file per call, named
...
timestamps are distinguished from test-supplied 2012 ones.  Prior-test
events Stripe finalizes after our cursor pin are dropped at normalize
time: their ``data.object.customer`` matches ``cus_UNRECORDED``
(no recorded ``Customer.create``) while our test's customer matches
    ...
    def _generate_and_save_stripe_fixture(*args: Any, **kwargs: Any) -> Any:
        ...
        try:
            ...
            with open(fixture_path, "w") as f:
                ...
                # vars(e) includes ``error``, which is a stripe.ErrorObject
                # (a StripeObject subclass that json.dumps doesn't know how
                # to serialize). read_stripe_fixture only consumes http_body
                # / http_status / headers, so coercing the rest to strings
                # via ``default=str`` is safe.
                f.write(
                    json.dumps(
                        error_dict,
                        indent=2,
...
def normalize_fixture_data(decorated_function: CallableT) -> None:  # nocoverage
    ...
    # ``cus_UNRECORDED`` suffix; the ``Event.list`` drop check below
    # uses that exact suffix to filter prior-test customers' webhook
    # events.  Other id placeholders use a generic
    # ``<prefix>_NORMALIZED`` -- no load-bearing meaning, just a
    # "this id was redacted" marker.  Customer ids we did record are
    # re-replaced with their fixture filename by the ``"id":`` pass
    # above.  We'll replace e.g. ``cus_D7OT2jf5YAtZQ2`` with
    # ``cus_UNRECORDED`` and ``txn_xxx`` with ``txn_NORMALIZED``.
    ...
    for fixture_file in fixture_files_for_function(decorated_function):
        ...
        # test card issuer assigns on the fly.
        file_content = re.sub(
            r'"authorization_code": "[0-9]+"', '"authorization_code": "000000"', file_content
        )
        # ``network_transaction_id`` is a per-charge 15-digit value
        # issued by the card network that Stripe just relays.
        file_content = re.sub(
            r'"network_transaction_id": "[0-9]+"',
            '"network_transaction_id": "000000000000000"',
            file_content,
        )
        # Idempotency keys appear both as the lowercase JSON field
        # (``"idempotency_key"``) and as the response-header field
        # (``"Idempotency-Key"``). Stripe assigns a fresh uuid for both
        # on every call.
        file_content = re.sub(
        ...
        # ``pending_webhooks`` counts how many of Stripe's webhook
        # subscribers have yet to receive this event; it ticks down as
        # Stripe delivers, so the same event captured a moment apart
        # has different values. Collapse to 0 so re-polled fixtures are
        # byte-identical.
        ...
        # ``cus_UNRECORDED`` -- the id catch-all stamps that placeholder
        # on any customer we never recorded a ``Customer.create`` for,
        # which for ``Event.list`` means a prior-test customer Stripe
        # finalized a webhook for after we anchored our cursor (and the
        # count of such leaked events varies regen to regen).  Our own
        # customer is re-replaced with the file-name string by the
        # ``"id":`` pass (because its id appeared as ``"id":`` in
        # ``Customer.create.<n>.json``), so the check unambiguously
