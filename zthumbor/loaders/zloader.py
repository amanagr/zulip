...
async def load(context, url):
    ...
    if source_type == THUMBOR_S3_TYPE:
        ...
        result = await s3_loader.load(context, actual_url)
        ...
        result = await file_loader.load(context, patched_local_url)
    elif source_type == THUMBOR_EXTERNAL_TYPE:
        result = await https_loader.load(context, actual_url)
